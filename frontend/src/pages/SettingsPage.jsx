...
// src/pages/SettingsPage.jsx - WITH WEBAUTHN FINGERPRINT
import React, { useEffect, useState, useRef } from 'react'
import { useNavigate } from 'react-router-dom'
import api from '../services/api'
import { 
...
function SettingsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setBiometricStatus({
          faceRegistered: bioRes.data.faceRegistered || false,
          fingerprintRegistered: bioRes.data.webAuthnRegistered || false,
        })
  ...
  // ✅ NEW: Register fingerprint using WebAuthn
  const registerFingerprint = async () => {
    try {
      ...
      const optionsResponse = await api.get('/biometric/fingerprint-register-options', {
        headers
      })

      console.log('Registration options received:', optionsResponse.data)

      // Step 2: Trigger device biometric registration
      const registrationResult = await startRegistration(optionsResponse.data)

      console.log('Device biometric registration completed:', registrationResult)
      ...
      if (verificationResponse.data.success) {
        ...
        setError(verificationResponse.data.message || 'Failed to register fingerprint.')
      }
  ...
  const getDeviceName = () => {
    ...
    if (userAgent.includes('iPhone')) return 'iPhone Touch ID'
    if (userAgent.includes('iPad')) return 'iPad Touch ID'
    if (userAgent.includes('Mac')) return 'Mac Touch ID'
    if (userAgent.includes('Windows')) return 'Windows Hello'
    if (userAgent.includes('Android')) return 'Android Fingerprint'
    ...
    <div className="min-h-screen bg-[#0D0D0D] text-white">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <section className="bg-[#101010] rounded-2xl border border-dashed border-gray-800 p-4 sm:p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Fingerprint Section - ✅ UPDATED WITH WEBAUTHN */}
            <div className="rounded-xl border border-dashed border-emerald-500/60 bg-[#141414] p-4">
              <div className="flex items-center gap-3 mb-3">
                <div className="h-10 w-10 rounded-full bg-emerald-500/20 flex items-center justify-center">
                  <svg className="h-5 w-5 text-emerald-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">Fingerprint</p>
                  <p className="text-xs text-gray-400">Touch ID / Face ID / Windows Hello</p>
                </div>
              </div>
              
              {biometricStatus.fingerprintRegistered ? (
                <div className="space-y-2">
                  <div className="text-xs text-emerald-300">✓ Biometric device registered</div>
                  <div className="text-xs text-gray-400">Ready for secure transactions</div>
                </div>
              ) : (
                <div className="space-y-2">
                  <p className="text-[11px] text-gray-400 mb-2">
                    Register your device biometric (fingerprint, Face ID, or Windows Hello) for high-value withdrawals.
                  </p>
                  <button
                    onClick={registerFingerprint}
                    ...
                    className="w-full px-4 py-2 text-xs rounded-lg bg-emerald-500/10 text-emerald-300 border border-dashed border-emerald-400/70 hover:bg-emerald-500/20 transition disabled:opacity-50"
                  >
                    {loading ? 'Registering...' : !browserSupportsWebAuthn() ? 'Not Supported' : 'Register Biometric'}
                  </button>
                </div>
              )}
            </div>

            {/* Face Section - UNCHANGED */}
            <div className="rounded-xl border border-dashed border-amber-500/70 bg-[#141414] p-4">
              <div className="flex items-center gap-3 mb-3">
                <div className="h-10 w-10 rounded-full bg-amber-500/20 flex items-center justify-center">
                  <svg className="h-5 w-5 text-amber-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
